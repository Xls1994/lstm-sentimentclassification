...
import data_helpers
datasets = {'imdb': (data_helpers.load_data, data_helpers.prepare_data)}
...
def adadelta(lr, tparams, grads, x_zheng, x_zheng_mask, x_ni, x_ni_mask, y, cost, cost1, cost2,cost3,cost4):
    """
    ...
    f_grad_shared_con = theano.function([x_zheng, x_zheng_mask, x_ni, x_ni_mask, y], cost3, updates=zgup + rg2up,
                                    name='adadelta_f_grad_shared_connect')
    f_grad_shared_crosslanguage = theano.function([x_zheng, x_zheng_mask, x_ni, x_ni_mask, y], cost4, updates=zgup + rg2up,
                                    name='adadelta_f_grad_shared_crosslanguage')
    ...
    return f_grad_shared, f_grad_shared_zheng, f_grad_shared_ni,f_grad_shared_con,f_grad_shared_crosslanguage, f_update

def build_model(alpha, beta, costselection,tparams, options):
    ...
    cost1 = -tensor.log(pred_zheng[tensor.arange(n_samples), y] + off).mean()
    cost2 = -tensor.log(pred_ni[tensor.arange(n_samples), y] + off).mean()

    cost3 = -tensor.log(pred[tensor.arange(n_samples), y] + off).mean()
    cost4 = tensor.sum(tensor.square(proj_zheng - proj_ni), axis=1).mean()

    if costselection=='alfa':
        print 'cos1,cos2,cos3'
        cost = alpha * (cost1 + cost2 + cost3) + beta * cost4
    elif costselection=='beta':
        print 'cos1,cos2'
        cost=alpha * (cost1 + cost2 )+beta*( cost3 +  cost4)
    elif costselection=='all':
...
def train_lstm(
    ...
    # reload_model=dataSetPath + 'lstm_model_'+category+'.npz'  # Path to a saved model we want to start from.
    ...
    (use_noise, x_zheng, x_zheng_mask, x_ni, x_ni_mask, y, f_pred_prob, f_pred,
     cost1, cost2, cost3, cost4, cost, f_proj) = build_model(alpha, beta,costselect, tparams, model_options)
    ...
    f_grad_shared, f_grad_shared_zheng, f_grad_shared_ni,f_grad_shared_con,f_grad_shared_crosslanguage, f_update \
        = optimizer(lr, tparams, grads, x_zheng, x_zheng_mask, x_ni, x_ni_mask, y, cost, cost1, cost2,cost3,cost4)
    ...
    try:
        ...
        for eidx in xrange(max_epochs):
            ...
            for _, train_index in kf:
                ...
                cost3=f_grad_shared_con(x1, x1_mask, x2, x2_mask, y)
                cost4=f_grad_shared_crosslanguage(x1, x1_mask, x2, x2_mask, y)
...
if __name__ == '__main__':
    ...
    categories = ['music']#'dvd', book
    dimensions = [100] # 100, 150, 250
    sentimentDim = 50
    ...
    SingleProcess(type,"book",100,sentimentDim,SerializerDir,TotalOutputDir,7,3,'alfa')
